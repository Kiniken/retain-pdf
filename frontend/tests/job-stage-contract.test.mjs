...
import { eventStageForMainStatus } from "../src/js/job-status/job-stage-event-selection.js";
import { latestMainStageEventRecord } from "../src/js/job-status/main-lane-stage-selection.js";
import { eventRecordForMainStatus } from "../src/js/job-status/canonical-stage-snapshot.js";
...
test("event-driven keepForwardStageKey clamps done while job is running", async () => {
  const { keepForwardStageKey } = await import(
    "../src/js/job-status/main-lane-stage-selection.js"
  );
  // Event payload says "done" — but the job itself has not reached succeeded yet.
  const stageKey = keepForwardStageKey(
    { job_id: "job-event-done-running", status: "running" },
    { display_stage: "done" },
    {},
    { hasCanonicalEventContract: true, canonicalDisplayStage: "done", publicStage: "done" },
  );
  assert.equal(stageKey, "render");

  const succeededStageKey = keepForwardStageKey(
    { job_id: "job-event-done-succeeded", status: "succeeded" },
    { display_stage: "done" },
    {},
    { hasCanonicalEventContract: true, canonicalDisplayStage: "done", publicStage: "done" },
  );
  assert.equal(succeededStageKey, "done");
});
...
test("eventStageForMainStatus exposes the status-card stage selection boundary", () => {
  const job = {
    job_id: "job-main-status-helper",
    status: "running",
    display_stage: "translation",
  };
  ...
  const missingDisplayStageEvent = {
    lane: "main",
    user_stage: "render",
    stage: "render_preprocess",
    substage: "render_prewarm",
  ...
  assert.equal(eventStageForMainStatus(job, backgroundRenderEvent, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  }), null);
  assert.equal(eventStageForMainStatus(job, missingDisplayStageEvent, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  }), null);

  const candidate = eventStageForMainStatus(job, translationEvent, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  });
  assert.equal(candidate.itemStageForMatch, "translate");
  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.progress.current, 4);
  assert.equal(candidate.progress.total, 10);
});

test("main lane stage selection consumes normalized records", () => {
  const job = {
    job_id: "job-normalized-main-selection",
    status: "running",
    display_stage: "translation",
  };
  const backgroundRecord = normalizedStageEventRecord({
    lane: "background",
    display_stage: "render",
    stage: "render_preprocess",
    substage: "render_prewarm",
  ...
  const missingDisplayStageRecord = normalizedStageEventRecord({
    lane: "main",
    user_stage: "render",
    stage: "render_preprocess",
    substage: "render_prewarm",
  ...
  const translationRecord = normalizedStageEventRecord({
    lane: "main",
    display_stage: "translation",
    stage: "render_preprocess",
    substage: "translation_batches",
    progress: { unit: "batch", current: 6, total: 10 },
  });

  assert.equal(eventRecordForMainStatus(job, backgroundRecord, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  }), null);
  assert.equal(eventRecordForMainStatus(job, missingDisplayStageRecord, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  }), null);

  const candidate = eventRecordForMainStatus(job, translationRecord, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  });
  assert.equal(candidate.itemStageForMatch, "translate");
  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.progress.current, 6);
  assert.equal(candidate.progress.total, 10);
});

test("canonical main stage candidate payload hides internal stage fields", () => {
  const job = {
    job_id: "job-canonical-candidate-payload",
    status: "running",
    display_stage: "translation",
  };
  const record = normalizedStageEventRecord({
  ...
  const candidate = eventRecordForMainStatus(job, record, {
    currentStageKey: "translate",
    publicJobStageKey: "translate",
  });

  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.payload.display_stage, "translation");
  assert.equal(candidate.payload.stage, "");
  assert.equal(candidate.payload.current_stage, "");
  assert.equal(candidate.payload.internal_stage, "");
  assert.equal(candidate.payload.stage_detail, "");
  assert.equal(candidate.payload.progress.unit, "batch");
  assert.equal(candidate.payload.progress.current, 6);
  assert.equal(candidate.payload.progress.total, 10);
});

test("canonical main stage selection does not use legacy render preprocess inference", () => {
  const job = {
    job_id: "job-canonical-main-no-legacy-preprocess",
    status: "running",
    display_stage: "translation",
  };
  const record = normalizedStageEventRecord({
  ...
  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.progress.current, 7);
});

test("latest main stage record ignores background render prewarm while translation is main", () => {
  const candidate = latestMainStageEventRecord(
    {
      job_id: "job-latest-main-record",
      status: "running",
      display_stage: "translation",
      stage: "translating",
    },
    {
      items: [
        ...
        },
        {
          ...
          stage: "render_preprocess",
          substage: "render_prewarm",
          progress: { unit: "step", current: 2, total: 3 },
        },
      ],
    },
  );
  ...
  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.progress.current, 30);
  assert.equal(candidate.progress.total, 100);
});

test("latest main stage desired substage uses canonical record before legacy payload", () => {
  const candidate = latestMainStageEventRecord(
    {
      job_id: "job-latest-main-record-substage",
      status: "running",
      display_stage: "translation",
      stage: "translating",
    },
      ...
      items: [
        {
          ...
          progress: { unit: "batch", current: 31, total: 100 },
        },
      ],
    },
  );
  ...
  assert.equal(candidate.itemStageKey, "translate");
  assert.equal(candidate.record.substage, "translation_batches");
  assert.equal(candidate.progress.current, 31);
});
