...
function findPageElementForSelection(root, selection = {}) {
...
function centerSelectionInScrollShell(root, pageElement, rect = {}) {
...
export function createReaderSelectionFavorites({
  ...
  function syncDrawer({ preserveScroll = false } = {}) {
  ...
  function updateFavoriteMetadata(id, patch = {}) {
    if (!id || !store?.save) {
      return;
    }
  ...
  function clearActiveSelection({ removeOverlay = false } = {}) {
    if (removeOverlay) {
  ...
  function setActiveSelection(selection = {}) {
    ...
    selection = { ...selection, rect: anchorRect, sourceRect, anchorRect, baseRect, scale };
    const overlay = createPinnedOverlay(root, documentRef);
    overlay.classList.add("is-final", "is-restored");
    updateSelectionOverlay(overlay, selection.rect);
    setOverlayPreview(overlay, selection.previewUrl);
    const nextSelection = {
      id: selection.id || selectionId(),
      mode: selection.mode || documentRef?.body?.dataset?.readerMode || "compare",
      overlay,
      page: selection.page,
      pane: selection.pane || paneFromPage(pageElement),
      pageElement,
      rect: selection.rect,
      anchorRect: selection.anchorRect,
      baseRect: selection.baseRect,
      sourceRect,
      scale: selection.scale,
      relativeRect: selection.relativeRect || relativeRect(sourceRect, pageElement),
      anchorRelativeRect: selection.anchorRelativeRect || viewportRelativeRect(anchorRect, root),
  ...
  function resolveFavoritePage(item = {}) {
  ...
  function beginPeekFavorite(item = {}) {
    if (peekState) {
      return;
    }
  ...
  function favoriteSelection(selection = activeSelection, { openDrawer = true } = {}) {
    if (!selection) {
      return [];
    }
    const item = {
