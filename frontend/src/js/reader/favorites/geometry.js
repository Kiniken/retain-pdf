...
export function relativeRect(rect = {}, pageElement) {
...
export function viewportRelativeRect(rect = {}, rootElement) {
...
export function rectFromViewportRelative(relative = {}, rootElement) {
  const viewportWidth = Number(globalThis.window?.innerWidth) || 0;
  const viewportHeight = Number(globalThis.window?.innerHeight) || 0;
  const rootRect = rootElement?.getBoundingClientRect?.();
...
export function rectFromRelative(relative = {}, pageElement) {
  const pageRect = pageElement?.getBoundingClientRect?.();
