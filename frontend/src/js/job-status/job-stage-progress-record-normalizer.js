...
function substageKeyForProgressRecord(record = {}, stageKey = "", payload = {}) {
  ...
  const payloadCandidates = [
    payload.substage,
    payload.payload?.substage,
    payload.stage,
    payload.current_stage,
    payload.internal_stage,
  ];
  for (const candidate of payloadCandidates) {
    const payloadSubstage = normalizeSubstageKey(candidate);
    if (payloadSubstage) {
...
export function normalizeProgressRecordFromEventRecord(job, record = {}, itemStage = "") {
