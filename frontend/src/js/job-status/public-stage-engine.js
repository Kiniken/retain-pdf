...
function stagePayloadForPresentation(job = {}, stageKey = "", substageKey = "", progress = {}) {
  return {
    ...
    lane: "main",
    progress: {
      unit: progress.unit || progress.progressUnit || "",
      current: progress.current ?? null,
      total: progress.total ?? null,
      percent: progress.percent ?? progress.progressPercent ?? progress.displayPercent ?? null,
    },
  };
}
...
function displayPercentFromProgress(progress = {}) {
  const explicit = progress.displayPercent ?? progress.progressPercent ?? null;
  ...
  if (Number.isFinite(current) && Number.isFinite(total) && total > 0 && progress.progressUnit === "percent") {
