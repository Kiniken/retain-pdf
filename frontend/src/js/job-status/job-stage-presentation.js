...
export function resolveDisplayedStagePresentation(job, eventsPayload) {
  ...
  const fallbackPublicStageKey = publicStageOf(job);
  ...
  const eventDisplayStage = publicStageOf(event) || normalizeUserStage(event.display_stage || event.payload?.display_stage || event.user_stage || event.payload?.user_stage || "");
  const eventProgressUnit = progressUnitOf(event);
  const rawEventPayload = {
    ...
    stage_detail: event.stage_detail || event.payload?.stage_detail || job.stage_detail || "",
    progress: {
      unit: eventProgressUnit,
      current: eventProgress.current,
      total: eventProgress.total,
    },
    progress_unit: eventProgressUnit,
  ...
  return {
    stageKey,
    stageKeyTrusted: Boolean(publicStageOf(job) || eventDisplayStage),
