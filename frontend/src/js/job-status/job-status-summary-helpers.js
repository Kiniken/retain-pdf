...
export function progressFromText(payload) {
  ...
  const englishProgressMatch = text.match(/\b(?:batch|batches|page|pages|step|steps)\s*(\d+)\s*[\/／]\s*(\d+)/i);
  if (englishProgressMatch) {
    return {
      current: numberOrNull(englishProgressMatch[1]),
      total: numberOrNull(englishProgressMatch[2]),
    };
  }
