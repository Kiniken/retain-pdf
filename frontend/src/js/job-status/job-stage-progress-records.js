...
export function stagePayloadFromEvent(job, item, progress) {
  ...
  return {
    ...
    stage_detail: item?.stage_detail || item?.payload?.stage_detail || "",
    progress: {
      unit: progressUnit,
...
export function collectLatestCurrentStageProgress(job, eventsPayload, stageKey = "", substageKey = "") {
  ...
  if (stageKey === "translate") {
    if (substageKey && substageKey !== "translation_batches") {
      return latestSameSubstage || latest;
    }
    if (substageKey === "translation_batches") {
      return latestSameSubstage || latest;
    }
    const latestTranslationBatches = collectLatestCurrentStageProgress(job, eventsPayload, stageKey, "translation_batches");
    return latestTranslationBatches || latestSameSubstage || latest;
  }
  return latestSameSubstage || latest;
}
...
export function collectStageProgressByKey(job, eventsPayload) {
  ...
  Object.entries(progressBySubstage).forEach(([stageKey, bySubstage]) => {
    const preferredProgress = stageKey === "translate" && bySubstage.translation_batches
      ? bySubstage.translation_batches
