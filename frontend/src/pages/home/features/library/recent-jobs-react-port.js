...
export function createRecentJobsReactViewPort({ store = createLibraryViewStore() } = {}) {
  const handlersRef = { current: { onOpen: null, onLoadMore: null, onSearch: null, isSuspended: () => false } };
  const autoLoadCheckerRef = { current: null };
  ...
  function renderLoading() {
    store.actions.setLoading();
  }

  function renderEmpty(message) {
    store.actions.setEmpty(message);
  }

  function renderError(message, { reset = false } = {}) {
    if (reset) {
      store.actions.setErrorReset(message);
      return;
    }
    // 镜像旧 applyRecentJobsErrorState 的 reset:false 分支:只清 load-more
    // 的加载态,不展示错误文案(错误提示走 error-box 通道,不在此越权渲染)。
    store.actions.clearLoadMoreLoading();
  }

  function renderList({ hasMore = false } = {}) {
    store.actions.setList(hasMore);
  }
  ...
  function setLoadMoreLoading() {
    store.actions.setLoadMoreLoading();
  }
  ...
  function scheduleAutoLoadCheck(options) {
  ...
  function registerAutoLoadChecker(checker) {
  ...
  function bindEvents({ onOpen, onLoadMore, onSearch, isSuspended = () => false } = {}) {
