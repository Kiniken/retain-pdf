...
function loadLottieWeb() {
  if (globalThis.window?.lottie) {
    return Promise.resolve(globalThis.window.lottie);
  }
  ...
  lottieLoaderPromise = new Promise((resolve, reject) => {
    ...
    script.async = true;
    script.onload = () => (globalThis.window?.lottie ? resolve(globalThis.window.lottie) : reject(new Error("lottie unavailable")));
...
export function useLottieStageAnimation(visualStageKey = "", progressSample = {}) {
  ...
  const { stageKey = "", current = NaN, total = NaN, progressUnit = "" } = progressSample || {};
  useEffect(() => {
