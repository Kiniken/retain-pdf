...
export function StageRetry({ selectedStageKey = "", action = null }) {
  ...
  if (!eligible) {
    return <div id={STATUS_CARD_IDS.stageRetry} className="status-stage-retry is-empty" aria-hidden="true" />;
  }
  const stage = action.stage || (selectedStageKey === "translate" ? "translation" : selectedStageKey);
  return (
    <div id={STATUS_CARD_IDS.stageRetry} className="status-stage-retry" aria-hidden="false">
      <button
        type="button"
