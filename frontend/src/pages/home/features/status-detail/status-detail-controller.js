...
export function createStatusDetailController({
  ...
  const resumeViewPort = {
    closeDialog: () => dialogStore.close(),
    setRerunAction: ({ enabled, status } = {}) => {
      store.actions.setOverview({ rerun: { enabled: Boolean(enabled), status: status || "" } });
    },
    setRerunDisabled: (disabled) => store.actions.setRerunPending(disabled),
  };
  ...
  async function applyTranslationFilter(query) {
  ...
  async function selectTranslationItem(itemId) {
    ...
    try {
      ...
      syncTranslation({ itemErrorText: error.message || String(error), itemDetailLoading: false });
    }
  }
  ...
  async function replayCurrentItem() {
    try {
      ...
      syncTranslation({ replayErrorText: error.message || String(error), replayLoading: false });
    }
  }
