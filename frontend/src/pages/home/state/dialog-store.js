...
//
// 参照 src/pages/reader/state/drawer-store.js 的模式(open/subscribe 契约),
...
export function createDialogStore(initialPayload = null) {
  let state = { open: false, payload: initialPayload };
  ...
  return {
    ...
    getState: () => state,
    open(payload = null) {
      state = { open: true, payload: payload === null ? state.payload : payload };
      notify();
      return state;
    },
    close() {
